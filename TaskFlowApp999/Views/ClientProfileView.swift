...
struct ClientProfileView: View {
    ...
    @State private var biometricStatus = ""
        ...
        NavigationView {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    // Biometric Status (only show if available)
    ...
    private var biometricStatusSection: some View {
        VStack(spacing: 12) {
            HStack {
                ...
                // Status indicator
                Circle()
                    .fill(authVM.isBiometricEnabled ? Color.green : Color.gray)
            ...
            Button(action: {
                ...
                Text("Manage Biometric Settings")
        ...
        VStack(spacing: 16) {
            ...
            VStack(spacing: 0) {
                ...
                // Biometric Authentication Option (only show if available)
                        if authVM.isBiometricAvailable {
                            ProfileMenuRow(
                                icon: authVM.biometricIcon,
                                title: "\(authVM.biometricName) Authentication",
                                subtitle: authVM.isBiometricEnabled ? "Enabled" : "Set up secure login"
                            ) {
                                showingBiometricSettings = true
                            }
    ...
    private func updateBiometricStatus() {
        if authVM.isBiometricAvailable {
            if authVM.isBiometricEnabled {
                biometricStatus = "\(authVM.biometricName) is enabled for secure login"
            } else {
                biometricStatus = "\(authVM.biometricName) is available but not enabled"
            }
        } else {
            biometricStatus = "Biometric authentication not available on this device"
        }
    }
